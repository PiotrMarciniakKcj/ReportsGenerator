# W: komentarze - kepiej za duzo niż za mało
...
from datetime import timedelta  # albo from datetime import datetime, timedelta... OR import datetime as dt
import csv
import codecs
...
# W: uporządkować importy


# !!!!! format and round  are doing the same thin
# convert float to string value with '%'
def float_to_percent(value):
    return ('{:.2f}%'.format(
        round((value * 100), 2))).replace('.', ',')


# return index of the last row of the table
def enumerate_sheet(worksheet, max_val):
    for i, i_row in enumerate(worksheet):
        if i_row[max_val].value == "Sum":  # uwaga na zaleznosc od tekstu raportu
            return str(i + 1)  # dlaczego + 1
        if i_row[max_val].value == "Suma":
            return str(i + 10)  # dlaczego + 10
...
def get_formatted_data(path, classification=False):
    ...
    if classification is False:  # if not classification    if classification
...
def format_data(indexes, path, classification=False):
    ...
    for row in input_table:
        ...
        row = temp_row
        if row[len(row) - 1] not in ("-", "%", ""):  # last index: row[-1]
            row[len(row) - 1] = float_to_percent(float(row[len(row) - 1]))  # czy float potrzebny?
        new_table.append(row)
    return new_table

# W: obsługa dat za pomocą biblioteki datetime
...
def get_date(filename):
    ...
    date = datetime.datetime(int(date[0:4]), int(date[4:6]), int(date[6:8]), int(date[9:11]), int(date[11:13]),
                             int(date[13:15]))
    return date  # W: zwrócić wartość zamiast przypisywać i zwracać
...
def paste_tables(formatted_data, table_name, is_summary=False, is_classification=False, is_cost323=False):
    for i, data_row in enumerate(formatted_data):  # W: zlikwidować enumerate
        if is_summary:
            for x in range(0, 2):  # W: range(2)
        ...
        else:  # W: dodać komentarz: tu jest detekcja
            table_row = table_name.add_row()
            table_row.height = Cm(0.5)
            table_row.height_rule = WD_ROW_HEIGHT_RULE.EXACTLY
            for x in range(0, len(data_row)):
...
# get weather  # W: komentarz dubluje nazwę funkcji
...
def get_last_sunday(date, input_month):
    ...
    if month[-1][6]:
        return month[-1][-2]  # W: co to robi? bo wygląda fajnie ;)
    else:
        return month[-2][-1]


# W: w razie czego biblitoeka: pytz
...
def get_test_start_text(date, point_x, point_y):
    ...
    # W:
    # return f'Rozpoczęcie testu: "{date.strftime("%d.%m.%Y")}", godz. " {...}...'
...
def format_document(document, DR_date, DP_date, N_date, order, summary, summary2=None):
    ...
    for paragraph in document.paragraphs:
        if 'order1' in paragraph.text:  # W: dodac komentarz co to jest ten order (że wartość z template)
...
def get_summary_tables(tables):
    ...
    for table in tables:
        if tables.index(table) % 2 == 0:
            N_sum += table[-1][2]  # W: opis - last row , 2 komorka
            em_sum += table[-1][3]
            ef_sum += table[-1][4]
        else:
            Nid_sum += table[-1][2]
...
# W: chyba bym to rozdzielil na fubnkcje get_detection_summarry_text (albo result_text) i uproscil procedure
...
def check_d_and_r(summary):
    if float(summary[0][0][3].replace(",", ".").replace("%", "")) < 99:
        return "System nie spełnia wymagań OPZ w zakresie poziomów detekcji."
    else:
        return "System spełnia wymagania OPZ w zakresie poziomów detekcji."
...
def generate_detection_and_identification_report(DR_excel, DP_excel, N_excel):
    ...
    output = template[0:-14] + " " + datetime.date.today().strftime("%Y") + ".docx"  # WW: fstring albo ''.format()
